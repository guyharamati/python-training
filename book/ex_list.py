...
def linear_merge(sorted1, sorted2):
    ...
    while True:
        ...
        if sorted1[cnt1] < sorted2[cnt2]:
            temp_sorted.append(sorted1[cnt1])
            cnt1 += 1
        else:
            temp_sorted.append(sorted2[cnt2])
            cnt2 += 1

    if minimum == 1:
        for i in xrange(cnt2, length2, 1):
            temp_sorted.append(sorted2[i])
    else:
        for i in xrange(cnt1, length1, 1):
            temp_sorted.append(sorted1[i])
    return temp_sorted
